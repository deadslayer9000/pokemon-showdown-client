...
abstract class BattleTypedSearch<T extends SearchType> {
	...
	protected formatType: 'doubles' | 'bdsp' | 'bdspdoubles' | 'rs' | 'frlg' | 'bw1' | 'letsgo' | 'metronome' | 'natdex' |
		...
		'svdlc1natdex' | 'stadium' | 'lc' | 'legendsza' | 'champions' | 'ddls3' | 'ghosttour' | 'ddls4' | 'jddldraft' | null = null;
	isDoubles = false;
	...
	constructor(searchType: T, format = '' as ID, speciesOrSet: ID | Dex.PokemonSet = '' as ID) {
		...
		if (format.includes('jddldraft') || format.includes('juicysdualtypedraftleague')) {
			this.formatType = 'jddldraft';
			this.dex = Dex.mod('jddldraft' as ID);
		}
		this.format = format;
	...
	protected firstLearnsetid(speciesid: ID) {
		...
		if (this.formatType === 'ddls4') table = table['ddls4patch'];
		if (this.formatType === 'jddldraft') table = table['jddldraft'];
	...
	getTier(pokemon: Dex.Species) {
		...
		const tableKey = this.formatType === 'doubles' ? `gen${gen}doubles` :
			...
			this.formatType === 'ddls4' ? 'ddls4patch' :
			this.formatType === 'jddldraft' ? 'jddldraft' :
...
class BattlePokemonSearch extends BattleTypedSearch<'pokemon'> {
	...
	getBaseResults(): SearchRow[] {
		...
		} else if (this.formatType === 'ddls4') {
			table = table['ddls4patch'];
		} else if (this.formatType === 'jddldraft') {
			table = table['jddldraft']
		}
...
class BattleMoveSearch extends BattleTypedSearch<'move'> {
	...
	static readonly GOOD_STATUS_MOVES = [
		'acidarmor', 'agility', 'aromatherapy', 'auroraveil', 'autotomize', 'banefulbunker', 'batonpass', 'bellydrum', 'blazeout', 'biteofwinter', 'bizarreelixir', 'bulkup', 'burningbulwark', 'calmmind', 'chillyreception', 'clangoroussoul', 'coil', 'colddeparture', 'cottonguard', 'courtchange', 'curse', 'defog', 'destinybond', 'detect', 'disable', 'dragondance', 'dragondrive', 'encore', 'enflame', 'extremeevoboost', 'filletaway', 'fluxemission', 'galecocoon', 'geomancy', 'glare', 'haze', 'healbell', 'healingwish', 'healorder', 'heartswap', 'heatsiphon', 'honeclaws', 'infernalshield', 'ironcladguard', 'kingsshield', 'leechseed', 'liftoff', 'lightscreen', 'lovelykiss', 'lunardance', 'magiccoat', 'maxguard', 'memento', 'milkdrink', 'moonlight', 'morningsun', 'nastyplot', 'naturesmadness', 'noretreat', 'obstruct', 'painsplit', 'partingshot', 'perishsong', 'piezoshift', 'protect', 'quiverdance', 'recover', 'reflect', 'reflecttype', 'rest', 'revivalblessing', 'roar', 'rockpolish', 'roost', 'sandbarrier', 'seasonalblessing', 'simulate', 'shedtail', 'shellsmash', 'shiftgear', 'shoreup', 'silktrap', 'slackoff', 'sleeppowder', 'sleeptalk', 'softboiled', 'solardance', 'spikes', 'spikyshield', 'spore', 'stealthrock', 'stickyweb', 'streamshift', 'strengthsap', 'substitute', 'switcheroo', 'swordsdance', 'synthesis', 'tailglow', 'tailwind', 'taunt', 'thunderwave', 'tidyup', 'toxic', 'transform', 'trick', 'victorydance', 'whirlwind', 'willowisp', 'wish', 'yawn',
	] as ID[] as readonly ID[];
	static readonly GOOD_WEAK_MOVES = [
		'accelerock', 'acrobatics', 'aquacutter', 'avalanche', 'barbaricincision', 'barbbarrage', 'bonemerang', 'bouncybubble', 'bulletpunch', 'buzzybuzz', 'ceaselessedge', 'chillingsilence', 'circlethrow', 'clearsmog', 'cogniblast', 'divinevolley', 'decisivebolt', 'doubleironbash', 'dragondarts', 'dragontail', 'drainingkiss', 'dualdivide', 'endeavor', 'effigybreak', 'facade', 'falseremedy', 'firefang', 'flipturn', 'flowertrick', 'freezedry', 'frustration', 'geargrind', 'gigadrain', 'grassknot', 'gyroball', 'hauntedrequiem', 'iaislash', 'icefang', 'iceshard', 'iciclespear', 'infernalparade', 'knockoff', 'lastrespects', 'lowkick', 'lumencascade', 'machpunch', 'mortalspin', 'mysticalpower', 'naturesmadness', 'nightshade', 'noxiouspower', 'noxiouswave', 'nuzzle', 'pikapapow', 'phantasmalvoyage', 'populationbomb', 'pulsarspiral', 'psychocut', 'psyshieldbash', 'pursuit', 'quickattack', 'quillstorm', 'ragefist', 'rapidspin', 'return', 'rockblast', 'ruination', 'saltcure', 'scorchingsands', 'searingclaws', 'seismictoss', 'shadowclaw', 'shadowsneak', 'sizzlyslide', 'solarflare', 'steelskewer', 'stoneaxe', 'storedpower', 'stormthrow', 'suckerpunch', 'superfang', 'surgingstrikes', 'tachyoncutter', 'tailslap', 'tomahawkvolley', 'thicketoutgrowth', 'thunderclap', 'tribeam', 'tripleaxel', 'tripledive', 'twinbeam', 'twincross', 'uturn', 'vacuumwave', 'veeveevolley', 'voltswitch', 'viralrestrain', 'watershuriken', 'weatherball', 'whiskaway', 'wretchedstab',
	] as ID[] as readonly ID[]; 
	static readonly BAD_STRONG_MOVES = [
		'belch', 'burnup', 'crushclaw', 'dragonrush', 'dreameater', 'eggbomb', 'firepledge', 'fissionblast', 'flyingpress', 'futuresight', 'grasspledge', 'hyperbeam', 'hyperfang', 'hyperspacehole', 'jawlock', 'landswrath', 'megakick', 'megapunch', 'mistyexplosion', 'muddywater', 'nightdaze', 'pollenpuff', 'rockclimb', 'selfdestruct', 'shelltrap', 'skyuppercut', 'slam', 'strength', 'submission', 'synchronoise', 'takedown', 'thrash', 'uproar', 'waterpledge',
	] as ID[] as readonly ID[];
	...
	getBaseResults() {
		...
		const regionBornLegality = dex.gen >= 6 &&
			(/^battle(spot|stadium|festival)/.test(format) || format.startsWith('bss') ||
				format.startsWith('vgc') || (dex.gen === 9 && this.formatType !== 'natdex' && this.formatType !== 'legendsza' && this.formatType !== 'ddls3' && this.formatType !== 'ghosttour' && this.formatType !== 'ddls4' && this.formatType !== 'jddldraft'));
	...
	if (this.formatType?.startsWith('bdsp')) lsetTable = lsetTable['gen8bdsp'];
		...
		if (this.formatType === 'ddls4') lsetTable = lsetTable['ddls4patch'];
		if (this.formatType === 'jddldraft') lsetTable = lsetTable['jddldraft'];
